...
const AchievementCard: React.FC<{ achievement: Achievement; theme?: string }> = React.memo(({ achievement, theme }) => {
  ...
  return (
    <div className={`relative p-4 sm:p-6 rounded-xl shadow-lg ring-1 flex flex-col transition-all duration-300 ease-in-out transform hover:scale-[1.02] touch-target
      ${isUnlocked 
    ...
    >
      {isUnlocked && category && (
        <span className={`absolute top-2 sm:top-3 right-2 sm:right-3 px-2 py-0.5 text-xs font-semibold rounded-full 
      ...
      <div className="flex items-center mb-3 sm:mb-4">
        <div className={`p-2 sm:p-3 rounded-lg mr-3 sm:mr-4 flex-shrink-0 ${isUnlocked ? (theme === 'dark' ? 'bg-gradient-to-br from-yellow-500 to-amber-600' : 'bg-gradient-to-br from-yellow-400 to-amber-500') : (theme === 'dark' ? 'bg-secondary-700' : 'bg-gray-200')}`}>
          <i className={`${icon} text-2xl sm:text-3xl ${isUnlocked ? 'text-white' : (theme === 'dark' ? 'text-gray-500' : 'text-gray-400')}`}></i>
        </div>
        <div className="flex-grow min-w-0">
          <h3 className={`text-lg sm:text-xl font-semibold leading-tight ${isUnlocked ? (theme === 'dark' ? 'text-white' : 'text-gray-900') : (theme === 'dark' ? 'text-gray-400' : 'text-gray-500')}`}>{title}</h3>
        </div>
      </div>
      <p className={`text-xs sm:text-sm mb-3 sm:mb-4 flex-grow leading-relaxed ${isUnlocked ? (theme === 'dark' ? 'text-gray-300' : 'text-gray-600') : (theme === 'dark' ? 'text-gray-500' : 'text-gray-400')}`}>{description}</p>
      {progress && (
        <div className="mb-3">
    ...
    <div className={`w-full h-full flex flex-col p-0 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
      <div className="mb-4 sm:mb-6 px-1 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div className="flex-grow">
          <h1 className={`text-2xl sm:text-3xl font-bold mb-1 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Başarılarım</h1>
          <p className={`text-sm sm:text-base ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
            {unlockedAchievements.length} / {achievements.length} başarı kilidi açıldı
          </p>
        </div>
        {onBack && (
          <button
            onClick={onBack}
            className={`px-3 sm:px-4 py-2 rounded-lg transition-colors flex items-center text-sm sm:text-base self-start sm:self-auto touch-target ${theme === 'dark' ? 'bg-secondary-700 hover:bg-secondary-600 text-gray-200' : 'bg-gray-200 hover:bg-gray-300 text-gray-700'}`}
