          ...
          return (
            <label
              ...
              className={`flex flex-col p-5 border-2 rounded-lg cursor-pointer transition-all duration-200 ease-in-out 
                                ${selectedType === type
                                  ? 'bg-primary-500 dark:bg-primary-600 border-primary-600 dark:border-primary-500 text-white shadow-xl scale-[1.02]'
                                  : isDisabled
                                    ? 'bg-gray-100 dark:bg-secondary-800 border-gray-300 dark:border-secondary-700 text-gray-400 dark:text-gray-500 opacity-60 cursor-not-allowed'
                                    : 'bg-white dark:bg-secondary-800 border-gray-300 dark:border-secondary-600 hover:border-primary-500 dark:hover:border-primary-400 hover:shadow-lg text-gray-700 dark:text-gray-200'
                                }`}
            title={isDisabled ? disabledText : title} // title'a disabledText'i ekledim, daha iyi erişilebilirlik
            >
              <div className="flex items-center mb-2">
                <div className={`p-2 rounded-md mr-3 text-lg ${selectedType === type ? 'bg-primary-400 dark:bg-primary-500' : isDisabled ? 'bg-gray-200 dark:bg-secondary-700' : 'bg-primary-100 dark:bg-primary-500/20'}`}>
                    <i className={`${icon} ${selectedType === type ? 'text-white' : isDisabled ? 'text-gray-400 dark:text-gray-600' : 'text-primary-600 dark:text-primary-400'}`}></i>
                </div>
                <h3 className={`font-semibold text-lg ${selectedType === type ? 'text-white' : 'text-gray-800 dark:text-white'}`}>{title}</h3>
                <input
                  type="radio"
                  id={`quiz-type-${type}`}
                  ...
                  className="ml-auto form-radio h-5 w-5 focus:ring-transparent focus:ring-offset-0 text-primary-300 dark:text-primary-200 bg-transparent border-transparent"
                  disabled={isDisabled}
                  style={selectedType === type ? { filter: 'brightness(2)'} : {}}
                  aria-label={title} // Erişilebilirlik için label eklendi
                />
              </div>
              <p className={`text-sm ${selectedType === type ? 'text-primary-100 dark:text-primary-200' : isDisabled ? 'text-gray-500 dark:text-gray-600' : 'text-gray-600 dark:text-gray-400'}`}>
                {description}
              </p>
              {isDisabled && <p className="mt-2 text-xs text-yellow-600 dark:text-yellow-400">{disabledText}</p>}
            </label>
          );
        })}
      </div>
